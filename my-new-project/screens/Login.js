...
export default class LoginScreen extends System {
  constructor(props) {
    super(props);
    this.state = {user: System.User ,email: '', password: ''};
  }
  ...
  _loginSubmit = async () =>{
    ...
    let id = await fetch('http://10.0.3.2:8080/demo/graph/login', {
        method: 'POST',
        headers: {
  ...
  render() {
    return (
      <View style={{flex:1,  padding: 25, }}>
        <Text style={{marginBottom: 25, color: 'lightblue', marginTop: 25}} onPress={()=>this.props.navigation.navigate("MyHome")}>Trang chủ</Text>
